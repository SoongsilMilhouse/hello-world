...
/*void mycpfrom(filesys *fs){
	...
	addSuperInode(fs,super_inode_num);
	addSuperData(fs,super_data,num);

	fs->inodeblock[super_inode_num].f_d='-';
	strcpy(fs->inodeblock[super_inode_num].time,timeSting());
	...
	if(fileSize()<128){
		...
		fs->inodeblock[super_inode_num].singleblock
/////////////////////////////////////////////하다말앗어
